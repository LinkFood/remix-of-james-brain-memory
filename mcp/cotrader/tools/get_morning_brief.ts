// get_morning_brief — Tier 1 v2 tool.
//
// Returns JAC's morning brief for a given ET date (default: today's brief if
// it has already fired, else yesterday's). Reads `brain_reports` filtered to
// `report_type='morning_brief'`, single row.
//
...
export async function getMorningBrief(args: GetMorningBriefArgs): Promise<GetMorningBriefResult> {
  ...
  const row = (data && data[0]) ? (data[0] as MorningBriefRow) : null;
  if (!row) {

/**
 * ct-detector-weekly-atm-voi (weekly_atm_voi_v1) — weekly DTE (1-7) + ATM
 * strike (±0.5%) + Vol/OI ≥ 1.0. Both call and put eligible.
 *
 ...
 * detector_id='weekly_atm_voi_v1' for any print whose contract is weekly,
 * struck within ±0.5% of underlying spot, and trading at Vol/OI ≥ 1.0.
 * Dedupes per option_symbol within config.dedupe_min minutes.
 *
 * Backtest (5-day corpus, 2026-04-20→24): 33% hit rate vs 22% base (1.49x
 * lift), 673 fires, 224 catches. The three discriminators that held 5/5 days.
 *
...
const DETECTOR_ID = 'weekly_atm_voi_v1';
...
interface Stats {
  ...
  alarms_skipped_dte: number;
  alarms_off_atm: number;
...
async function runSweep(supabase: SupabaseClient): Promise<Stats> {
  ...
  const dteMin = typeof cfg.dte_min === 'number' ? cfg.dte_min : 1;
  const dteMax = typeof cfg.dte_max === 'number' ? cfg.dte_max : 7;
  const strikeBandPct = typeof cfg.strike_band_pct === 'number' ? cfg.strike_band_pct : 0.005;
  const minVoiRatio = typeof cfg.min_voi_ratio === 'number' ? cfg.min_voi_ratio : 1.0;
  ...
  for (let offset = 0; offset < SCAN_LIMIT; offset += PAGE_SIZE) {
    ...
    const { data: page, error } = await supabase
      .from('ct_flow_alerts')
      .select('alert_id, ticker, side, strike, expiry, premium, price, volume, open_interest, ingested_at, underlying_price')
  ...
  for (const a of alerts) {
    try {
      ...
      // Both call and put eligible — this detector takes both directions.
      ...
      if (
        ...
        underlying == null || !Number.isFinite(underlying) || underlying <= 0
      ) {
        continue;
      }

      // DTE: weekly only (1-7 calendar days).
      ...
      if (dte < dteMin || dte > dteMax) {
        stats.alarms_skipped_dte += 1;
        continue;
      }

      // ATM band: |strike - underlying| / underlying ≤ strike_band_pct.
      const strikePctOff = Math.abs(strike - underlying) / underlying;
      if (strikePctOff > strikeBandPct) {
        stats.alarms_off_atm += 1;
        continue;
      }

      // V/OI ≥ min_voi_ratio. Both must be present and positive.
      ...
      const strikePctOffStr = (strikePctOff * 100).toFixed(2);
      const thesis = `${ticker} ${side} $${strike} ${expiry} — ATM (${strikePctOffStr}% off spot), V/OI=${voiRatio.toFixed(1)}x, ${dte}DTE`;
      ...
      const { error: flagErr } = await supabase
        ...
        .insert({
          ...
          direction,
          score: 0,
          tags: ['detector_alarm', DETECTOR_ID],
          thesis,

-- Re-define ct_specialist_outcome_stats to join on ct_contract_tracks.print_time
-- instead of first_tracked_at. first_tracked_at is when the print-grader CREATED
-- the row (often hours later in a backfill sweep) and was mis-windowing every
-- join — first deploy showed flags_with_track=0 across all specialists despite
-- 41 flags + 100+ matured tracks. print_time is when the underlying tape print
-- happened, which is the moment specialists actually flag against.

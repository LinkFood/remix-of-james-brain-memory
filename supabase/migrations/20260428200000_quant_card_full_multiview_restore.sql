-- =============================================================================
-- 20260428200000 — Restore the full multi-view quant card.
--
-- Migration 20260424000009 ("fix gamma_flip only") accidentally CREATE OR
-- REPLACE'd build_ticker_quant_card with a stripped-down version that emits
-- only `structure` + `flow_last_hour` and lost everything else: sentiment,
-- positioning, macro, events, news, brief, historical, freshness, plus the
-- legacy aliases (structural, dark_pool_last_hour, greek_flow_last_hour).
-- That's been in production for 4 days. Specialists, detectors, and
-- scoreboards have been reading null for put_call_ratio, earnings dates,
-- recent news, brief tilt, insider/political/analyst data, VIX context.
--
-- This migration restores the multi-view body from 20260422000019 and
-- inlines the correct gamma_flip computation (sign-change detection via
-- ct_compute_gamma_flip, which is the proven helper from 20260423000007).
-- After this, build_ticker_quant_card is a single source of truth — no
-- decorator chain, no `_orig` indirection.
-- =============================================================================
...
BEGIN
  ...
  -- -----------------------------------------------------------------------
  -- STRUCTURAL view — gamma_flip via sign-change helper (correct algorithm).
  -- -----------------------------------------------------------------------
  ...
  -- -----------------------------------------------------------------------
  -- Legacy aliases — preserve Wave G field names for downstream consumers.
  -- -----------------------------------------------------------------------
  BEGIN
    SELECT jsonb_build_object(
...
COMMENT ON FUNCTION public.build_ticker_quant_card IS
  'v3 (2026-04-28): full multi-view restored after 4-day regression from 20260424000009. gamma_flip uses ct_compute_gamma_flip (sign-change detection) inlined directly. Single source of truth — no decorator chain.';

-- Drop dark_pool_last_hour from quant card decorator output.
-- 2026-04-23: DP signal retired (unreliable direction). Historical data
-- stays in ct_dark_pool_prints for lookback but Claude no longer sees it
-- in his per-tick quant card.
...
BEGIN
  ...
  -- Strip dark-pool fields from result regardless of orig output
  IF v_result ? 'dark_pool_last_hour' THEN
    v_result := v_result - 'dark_pool_last_hour';
  END IF;
  IF v_result ? 'flow_last_hour' AND (v_result->'flow_last_hour') ? 'dp_accumulation' THEN
    v_result := jsonb_set(v_result, '{flow_last_hour}',
      (v_result->'flow_last_hour') - 'dp_accumulation', false);
  END IF;
  ...
  -- Strip dp_accumulation again in case v_flow_live re-introduced it
  IF v_result ? 'flow_last_hour' AND (v_result->'flow_last_hour') ? 'dp_accumulation' THEN
    v_result := jsonb_set(v_result, '{flow_last_hour}',
      (v_result->'flow_last_hour') - 'dp_accumulation', false);
  END IF;

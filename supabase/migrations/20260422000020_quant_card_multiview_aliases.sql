-- Wave N.3 supplement: re-apply build_ticker_quant_card with legacy aliases.
-- The prior migration went through before the legacy-shape block was added.
-- This file re-runs CREATE OR REPLACE FUNCTION so consumers relying on the
-- Wave G field names (dark_pool_last_hour, greek_flow_last_hour, events, news,
-- structure, sentiment.net_premium_cum) continue to work.

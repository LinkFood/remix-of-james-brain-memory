-- T+1 OI direction-confirmation wiring into ct_score_flow_event.
--
-- Until this migration, the 20-point `t1_oi_confirm` slot in the scorer was
-- dead code — always 0. MFE calibration proved the existing scorer is a
-- VOLATILITY detector (MFE ≈ MAE across every band), not a DIRECTION
-- detector. Overnight OI change is the missing discriminator per academic
-- consensus (Lakonishok et al., Augustin et al.): positions held overnight
-- cost margin and reveal conviction in a way intraday flow cannot.
--
-- Rule:
--   For alerts whose event_ts is at least 18 hours old (enough for the
--   overnight OI snapshot to settle), look up ct_oi_snapshots for the exact
--   option_symbol, read oi_delta_1d, and award:
--     opening_buy  with oi_delta_1d >= +100   → +20  (confirmed direction)
--     opening_sell with oi_delta_1d <= -100   → +20
--     opening_buy  with oi_delta_1d <= -100   → -10  (contradiction penalty)
--     opening_sell with oi_delta_1d >= +100   → -10
--     otherwise (too recent / no snapshot / |delta| < 100 contracts) → 0
--
-- Noise floor of 100 contracts filters snap-to-snap churn.
--
-- Matching: option_symbol is a full OCC symbol, globally unique → we match
-- on it alone. We do NOT rely on ct_oi_snapshots.side (historically stored
-- inconsistently as 'C' / 'call'); the option_symbol carries the side.
--
-- score_breakdown gains a `t1_oi` JSONB block recording the contribution,
-- the oi_delta_1d value, the matched snapshot id, and a status tag
-- (confirmed / contradicted / unavailable).
...
BEGIN
  ...
  -- Only consult once the alert is at least 18h old: snapshots are taken
  -- open/mid/close so we want at least one post-event snapshot to have
  -- been captured and ct_compute_oi_deltas to have populated oi_delta_1d.

-- =============================================================================
-- 20260422000019_quant_card_multiview.sql
--
-- Wave N.3: rewrite build_ticker_quant_card as a MULTI-VIEW card.
--
-- Previously the card was flow/gamma-heavy with sentiment + events + news as
-- afterthoughts. Tenet 13 (ungated signal access) requires every category to
-- stand on its own at equal weight so Claude can follow positioning or macro
-- or sentiment without having to dig.
--
-- New top-level shape (each section stands on its own — NULL if empty, never
-- raises the whole RPC):
--
--   {
--     ticker, as_of, generation_id, generation_days_remaining,
--     structural:  { spot, gamma_flip, call_wall, put_wall, max_pain,
--                    iv_rank, iv_percentile, net_gamma, regime },
--     flow_last_hour:     { net_call_prem, net_put_prem, whales[], sweep_count,
--                           dp_accumulation, greek_flow_delta, greek_flow_vega },
--     positioning:        { insider_trades_last_30d[], political_trades_last_30d[],
--                           analyst_actions_last_7d[], institutional_changes_last_90d[],
--                           short_interest },
--     macro:              { vix_latest, yield_curve_snapshot, central_bank_state,
--                           correlations, sector_tide_today, breaking_news_last_24h[] },
--     sentiment:          { nope_latest, put_call_ratio, net_premium_cum_today,
--                           prediction_market_overlay, recent_news[] },
--     historical:         { seasonality, earnings_history, next_earnings,
--                           recent_indicator_events },
--     brief:              { tilt, focus_level, avoid_today, source_brief_id },
--     freshness:          { snapshot_at, seconds_stale }
--   }
--
-- Claude can cite any field with path notation, e.g.
--   NVDA.positioning.insider_trades_last_30d[0].insider_title
--
-- Wave N.2 tables (ct_prediction_markets, ct_yield_curve, ct_correlations,
-- ct_seasonality, ct_central_bank_rates, ct_institutional_holdings,
-- ct_indicator_events) may not exist yet. The RPC is wrapped so each reference
-- returns NULL gracefully via to_regclass() guards and EXECUTE format() shims.
-- Once Wave N.2 lands, the reads activate without needing a new migration.
-- =============================================================================

SET search_path = public, extensions;

-- ---------------------------------------------------------------------------
-- Helper: read most-recent row (jsonb) from an optional table matching a
-- single-ticker or table-wide filter. Returns NULL if table doesn't exist.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public._qc_maybe_read(
  p_sql text
) RETURNS jsonb
...
END;
$fn$;
GRANT EXECUTE ON FUNCTION public._qc_maybe_read(text) TO authenticated, service_role;
...
DECLARE
  ...
  -- Legacy shape aliases for Wave G consumers (ct-trade-idea-generator,
  -- ct-ticker-snapshot-builder). Populated below and merged into the return.
  v_legacy_flow              jsonb;
  v_legacy_dp                jsonb;
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'gamma_flip',    (SELECT strike FROM gamma_flip),
  ...
  SELECT jsonb_build_object(
    ...
    'put_call_ratio',            CASE
      WHEN COALESCE((SELECT call_vol_cum FROM np_cum),0) = 0 THEN NULL
      ELSE (SELECT put_vol_cum / NULLIF(call_vol_cum,0) FROM np_cum)
    END,
    'net_premium_cum_today',     COALESCE((SELECT net_premium_cum_today FROM np_cum), 0),
    -- Legacy alias for N.4-owned consumers that still read net_premium_cum.
  ...
  -- -----------------------------------------------------------------------
  -- Legacy aliases — preserve the Wave G field names that existing consumers
  -- (ct-trade-idea-generator, ct-ticker-snapshot-builder) already read. The
  -- new multi-view shape is a superset; the aliases are thin re-pointers so
  -- N.4's execution layer keeps working without a coordinated cutover.
  -- -----------------------------------------------------------------------
  -- Build legacy aliases. No nested DECLARE — vars already hoisted.
  BEGIN
    -- flow_last_hour legacy extras (alert_count, total_premium, whale_count,
    -- opening_trades) — re-derive from ct_flow_alerts in one pass.
    SELECT jsonb_build_object(
    ...
    -- dark_pool_last_hour legacy shape.
    SELECT jsonb_build_object(
    ...
    -- greek_flow_last_hour legacy shape.
    SELECT jsonb_build_object(
    ...
    -- events legacy shape (next_earnings + upcoming_catalysts)
    SELECT jsonb_build_object(
    ...
    -- Note: v_sentiment itself already includes the legacy `net_premium_cum`
    -- alias alongside `net_premium_cum_today`; v_brief carries avoid_today +
    -- tilt + source_brief_id which is what downstream consumers actually use.
  END;

  RETURN jsonb_build_object(
    -- Wave N.3 multi-view surface (primary)
    'ticker',                    v_ticker,
    'as_of',                     v_as_of,
    ...
    -- Legacy aliases (Wave G shape) — kept so N.4-owned downstream
    -- consumers (ct-trade-idea-generator, ct-ticker-snapshot-builder) don't
    -- break. Alias the new sections back to the old top-level names.
